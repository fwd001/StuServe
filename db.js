...
module.exports = {
	...
	login: (payload, callback) => {
		...
		MongoClient.connect(url, { useNewUrlParser: true }, (err, client) => {
			...
			var db = client.db(dbName)
			// 插入数据
			db.collection(user).findOne({ username, password }, function(
				err,
				result
			) {
	...
	add: (payload, callback) => {
		...
		MongoClient.connect(url, { useNewUrlParser: true }, (err, client) => {
			...
			db.collection(list).insertOne(
				{ name, age, chinese, english, mathematics, id },
				(err, result) => {
					if (err) return console.log('查询list数据库库失败', err)
					callback(result)
				}
	...
	del: (payload, callback) => {
		...
		MongoClient.connect(url, { useNewUrlParser: true }, (err, client) => {
			...
			var db = client.db(dbName)
			// 删除一条数据
			db.collection(list).deleteOne({ _id: new ObjectID(_id) }, function(
				err,
				result
			) {
				if (err) return console.log('删除list数据失败了', err)
				if (result.result.ok === 1) callback()
			})
			// 关闭连接数据库
			client.close()
		})
	...
	edit: (payload, callback) => {
		const { _id, name, chinese, english, mathematics, age } = payload
		MongoClient.connect(url, { useNewUrlParser: true }, (err, client) => {
			if (err) return console.log('连接数据失败', err)
			var db = client.db(dbName)
			// 删除一条数据
			db.collection(list).updateOne(
				...
				{ $set: { name, chinese, english, mathematics, age } },
				function(err, result) {
					if (err) return console.log('编辑list数据失败了', err)
          if (result.result.ok === 1) callback()
				}
			)
			// 关闭连接数据库
